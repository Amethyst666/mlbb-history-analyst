...
class DatabaseHelper {
  ...
  Future<bool> isGameExists(String matchId) async {
    ...
    final res = await db.query('games', where: 'match_id = ?', whereArgs: [matchId]);
  ...
  Future<int> getOrCreateProfile(String nickname, {bool isUser = false, String? gameAccountId, int serverId = 0}) async {
    ...
    if (gameAccountId != null && gameAccountId.isNotEmpty && gameAccountId != '0') {
      final byId = await db.query('player_profiles', columns: ['id', 'main_nickname'], where: 'game_account_id = ?', whereArgs: [gameAccountId]);
      if (byId.isNotEmpty) {
    ...
    if (profileId == null) {
      final mapping = await db.query('player_nicknames', columns: ['profile_id'], where: 'LOWER(nickname) = ?', whereArgs: [cleanNick.toLowerCase()]);
    ...
    if (profileId == null) {
      int? customId = (gameAccountId != null) ? int.tryParse(gameAccountId) : null;
      if (customId != null && customId <= 0) customId = null;
      
      try {
        profileId = await db.insert('player_profiles', {
          if (customId != null) 'id': customId,
          'main_nickname': cleanNick, 'is_user': isUser ? 1 : 0,
          'game_account_id': gameAccountId, 'server_id': serverId
        });
      } catch (_) {
        profileId = await db.insert('player_profiles', {
          'main_nickname': cleanNick, 'is_user': isUser ? 1 : 0,
      ...
      await db.update('player_profiles', {'is_user': 1}, where: 'id = ?', whereArgs: [profileId]);
    }
    return profileId!;
  }
  ...
  Future<void> deleteAlias(int profileId, String alias) async {
    ...
    final profile = await db.query('player_profiles', columns: ['pinned_alias'], where: 'id = ?', whereArgs: [profileId]);
  ...
  Future<void> addComment(int profileId, String text) async {
    Database db = await database;
    await db.insert('player_comments', {'profile_id': profileId, 'comment': text.trim(), 'timestamp': DateTime.now().toIso8601String()});
    updateNotifier.notifyListeners();
  }
  ...
  Future<List<PlayerStats>> getPlayersForGame(int gameId) async {
    ...
    return List.generate(maps.length, (i) {
      ...
      map['nickname'] = map['profile_pinned_alias'] ?? map['profile_main_name'] ?? map['nickname'];
      map['is_user'] = ((map['profile_is_user'] == 1) || (map['is_user'] == 1)) ? 1 : 0;
  ...
  Future<int> _getOrCreateProfileTxn(Transaction txn, String nickname, {bool? forceIsUser, String? gameAccountId, int serverId = 0}) async {
    ...
    if (gameAccountId != null && gameAccountId.isNotEmpty && gameAccountId != '0') {
      final byId = await txn.query('player_profiles', columns: ['id', 'main_nickname'], where: 'game_account_id = ?', whereArgs: [gameAccountId]);
      if (byId.isNotEmpty) {
        profileId = byId.first['id'] as int;
    ...
    if (profileId == null) {
      final mapping = await txn.query('player_nicknames', columns: ['profile_id'], where: 'LOWER(nickname) = ?', whereArgs: [cleanNick.toLowerCase()]);
      if (mapping.isNotEmpty) {
        profileId = mapping.first['profile_id'] as int;
    ...
    if (profileId == null) {
      ...
      try {
        profileId = await txn.insert('player_profiles', {
          if (customId != null) 'id': customId,
          'main_nickname': cleanNick, 'is_user': forceIsUser == true ? 1 : 0,
          'game_account_id': gameAccountId, 'server_id': serverId
        });
      } catch (_) {
        profileId = await txn.insert('player_profiles', {
      ...
      await txn.update('player_profiles', {'is_user': 1}, where: 'id = ?', whereArgs: [profileId]);
    }
    return profileId!;
  }
  ...
  Future<List<GameStats>> getGames() async {
    Database db = await database;
    final maps = await db.query('games', orderBy: 'COALESCE(end_date, date) DESC');
  ...
  Future<List<GameStats>> getGamesByHero(int heroId) async {
    Database db = await database;
    final maps = await db.query('games', where: 'hero = ?', whereArgs: [heroId.toString()], orderBy: 'COALESCE(end_date, date) DESC');
    return maps.map((m) => GameStats.fromMap(m)).toList();
  }
}